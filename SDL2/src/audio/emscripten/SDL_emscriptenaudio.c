...
FeedAudioDevice(_THIS, const void *buf, const int buflen)
{
    ...
    EM_ASM_ARGS({
        ...
        for (var c = 0; c < numChannels; ++c) {
            if (typeof SDL2.audio.currentOutputBuffer['copyToChannel'] === "function") {
              var channelData=new Float32Array($1);
              for (var j = 0; j < $1; ++j) {
                  channelData[j] = HEAPF32[$0 + ((j*numChannels + c) << 2) >> 2];  /* !!! FIXME: why are these shifts here? */
              }
              SDL2.audio.currentOutputBuffer['copyToChannel'](channelData,c);
            } else {
